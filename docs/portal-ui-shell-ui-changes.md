...
> **说明**：按产品要求，本次重构**不在 UI 上新增功能区块**（例如 PRD §5.3 提到的当月签到日历、Turnstile 控件、返利划转面板等均未加入）。**用户可见文案保持重构前原样**（2026-06-10 已还原误改的 toast、标签、描述等）。下文列的是对**现有界面**的调整与删减。
...
| 签到说明 | ~~文案改为「奖励由 NewAPI 直接发放到账户」~~ **已还原**为「奖励直接加到你的账户里。」 |
| 签到 Toast | 成功且 `quotaAmount > 0` 时：`签到完成，获得 {余额格式}`；重复签到：`今日已签到` |
| 快捷入口 | ~~「刷新余额」~~ **已还原**为「查充值记录。」 |
...
| Toast | ~~支付返回 / 创建支付文案~~ **已还原**为「正在刷新余额与充值记录…」「订单已创建，正在前往支付」 |
...
| 区域 | 变更 |
|------|------|
| URL 参数 | 仅 `aff_code` 与 `ref`（**已移除** `inviteCode` 兼容） |
| 提交 body | 字段 `affCode`（BFF 不再接受 `inviteCode`） |
| 文案 | ~~标签「邀请码（aff_code）」~~ **已还原**为「邀请码 (可选)」；placeholder 还原为「如有邀请码可填写」 |

### 为什么改

- **Phase 2**：邀请体系统一为 NewAPI `aff_code`；旧 `?inviteCode=` 链接不再支持（v2 PRD §4.2）。

### 用户可见影响

- 邀请链接须使用 `aff_code`；旧 `inviteCode` 链接无效。
...
详见 [portal-ui-shell-prd.md](./portal-ui-shell-prd.md) v2.0 与 [newapi-contract.md](./newapi-contract.md)。
