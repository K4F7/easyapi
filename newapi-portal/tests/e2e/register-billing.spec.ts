...
test.describe("Billing referral", () => {
  ...
  test("invite link uses inviteCode, copy button, steps, and reward tab", async ({
    ...
    await registerPage.getByLabel("用户名").fill("invitee");
    await registerPage.getByLabel("邮箱").fill("invitee@example.com");
    await registerPage.getByLabel("密码", { exact: true }).fill("InvitePass99!");
    await registerPage.getByLabel("确认密码").fill("InvitePass99!");
    await registerPage.getByLabel("验证码").fill("123456");
    await registerPage.getByRole("button", { name: "注册", exact: true }).click();
