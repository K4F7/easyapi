...
async function openPlaygroundChat(page: Page) {
  ...
  await expect(page.getByRole("button", { name: /gpt-test-model|选择模型/ })).toBeVisible({
    timeout: 15_000,
  });
}
...
test.describe("Playground", () => {
  ...
  test("sidebar shows 操练场 between 用量 and 个人", async ({ page }) => {
    const labels = await page.getByRole("navigation").getByRole("link").allTextContents();
  ...
  test("defaults to chat tab; tab switches update URL without full reload", async ({
    ...
    await expect(page.getByRole("tab", { name: "对话", selected: true })).toBeVisible();
    ...
    await expect(page.getByText("生图功能即将上线")).toBeVisible();
  ...
  test("image tab placeholder has no API key in URL", async ({ page }) => {
  ...
  test("empty token list prompts creating a token", async ({ page }) => {
    ...
    await page.goto("/dashboard/playground");
    await expect(page.getByRole("link", { name: /去「令牌」页创建/ })).toHaveAttribute(
      "href",
      "/dashboard/tokens",
    );
  });
  ...
  test("token selector shows name and masked key only", async ({ page }) => {
    await mockTokens(page, MASKED_TOKENS);
    await page.goto("/dashboard/playground");
    await expect(page.getByRole("button", { name: /E2E Token A/ })).toBeVisible();
  ...
  test("chat: stop keeps partial content; clear needs confirmation", async ({
    ...
    await expect(page.getByRole("heading", { name: "清空对话？" })).toBeVisible();
  ...
  test("chat: switching token resets unavailable model", async ({ page }) => {
    await openPlaygroundChat(page);
    await expect(page.getByRole("button", { name: /gpt-test-model/ })).toBeVisible();
    ...
    await expect(page.getByRole("button", { name: /gpt-test-model/ })).toHaveCount(0);
  });
  ...
  test("chat: upstream errors are sanitized", async ({ page }) => {
    ...
    await textarea.press("Enter");
    await expect(page.getByText("上游对话接口返回错误，请稍后重试")).toBeVisible();
  ...
  test("navigation between dashboard pages has no client errors", async ({
    ...
    await openPlaygroundChat(page);
    await page.reload();
    await expect(page.getByRole("heading", { name: "操练场", level: 1 })).toBeVisible();
